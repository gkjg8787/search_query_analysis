...
- 結果の"url_template"の値が固定({}がない)または入力したurlと同じ値の場合、解析に失敗しているか検索テキストや検索ボタンを見つけられていない or 間違ったものを使用している可能性がある。
  - 動的なサイトの場合、待ち時間が短すぎて描画前にHTMLの解析 or クリックしようとしている可能性があり、待ち時間を長くすれば解決する場合がある。
    - downloader.pyの待ち時間の設定 DEFAULT_WAIT_TIME 変数を大きくする。
  - cookieが必要なサイトの場合、cookieのsave, load を true にし、2回試行（一度アクセスしてcookieを作成、もう一度アクセス）すればうまく行く場合がある。
